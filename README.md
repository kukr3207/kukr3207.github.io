...
Alignerr describes the confirmed long-horizon evaluation workflow: an initial build prompt for two models, rubrics and rationales based on outputs and trajectories, and follow-up prompts until either model exceeds 1M cumulative tokens from the initial prompt. This threshold applies to one model's cumulative run, not a combined total or a context-window size. The card links to its workflow at `/case-studies/verifier-design/#alignerr`.

Toptal describes the completed technical operator engagement using the supplied role details: exploratory computer-use tasks, interaction-data collection, quality validation and refinement of exploration methods. Its card links to `/case-studies/verifier-design/#toptal`. The summary does not turn the billing period into employment dates or infer a language stack from the job's required skills. Client identifiers, internal project titles and compensation details are omitted.

Project summaries omit private deliverables and internal task names. Confirmed reported metrics appear on the relevant previews; ambiguous revenue and deployment figures remain omitted.
